...
a source word x and a target word y. The rule is that if x starts
at position i in the original string S, then we will replace that
...
You only need to complete the function findAndReplace()
that takes a string S, an integer Q, and 3 arrays index, sources,
and targets of size Q, as input and returns the new string after all
the operations. index[i], sources[i], and targets[i] denotes the index,
sources, and targets for ith query.
...
class Solution:
    def findAndReplace(self,S,Q,index,sources,targets):
        ...
        for _ in range(Q):
            ...
            s=sources[j]
            t=targets[j]
            if(x[i:i+len(s)]==list(s)):
                z=x[i:i+len(s)]
                x=x[:i]+list(t)+x[i+len(s):]
        return ''.join(x)
