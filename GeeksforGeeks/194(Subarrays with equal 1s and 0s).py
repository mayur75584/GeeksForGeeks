'''
Subarrays with equal 1s and 0s

Given an array containing 0s and 1s.
Find the number of subarrays having equal number of 0s and 1s.

Example 1:

Input:
n = 7
A[] = {1,0,0,1,0,1,1}
Output: 8
Explanation: The index range for the 8
sub-arrays are: (0, 1), (2, 3), (0, 3), (3, 4),
(4, 5) ,(2, 5), (0, 5), (1, 6)

Example 2:

Input:
n = 5
A[] = {1,1,1,1,0}
Output: 1
Explanation: The index range for the
subarray is (3,4).

Your Task:
You don't need to read input or print anything. Your task is to complete the function countSubarrWithEqualZeroAndOne()
which takes the array arr[] and the size of the array as inputs and returns the number of subarrays with equal number
of 0s and 1s.

Expected Time Complexity: O(n).
Expected Auxiliary Space: O(n).

Constraints:
1 <= n <= 106
0 <= A[i] <= 1
'''
class Solution:
    def countSubarrayWithEqualZeroAndOne(self,n,arr):
        #using a map to store frequency of values obtained for prefix sum.
        mp={}
        curr_sum=0
        count=0
        for i in range(n):
            #calculating prefix sum by considering 0 as -1 and adding 1 if
            #value of arr[i] is 1 or adding -1 if it is 0.
            if arr[i]==0:
                curr_sum+=-1
            else:
                curr_sum+=arr[i]

            #updating the value in map
            mp[curr_sum]=mp.get(curr_sum,0)+1
        for key,value in mp.items():
            #if value of any element in map is more than 1, then we
            #update the count of subarrays.
            #count=(value*(value-1))/2 where value is value of key in map.
            if value>1:
                count+=(value*(value-1))//2

        #adding frequency of sum 0 in the map to the final result
        if 0 in mp.keys():
            count+=mp[0]
        return count

if __name__ == '__main__':
    # T=int(input())
    T=1
    while(T>0):
        # n=int(input())
        # A=list(map(int,input().split()))
        n=7
        A=[1,0,0,1,0,1,1]
        ob=Solution()
        res=ob.countSubarrayWithEqualZeroAndOne(n,A)
        print(res)
        T-=1




